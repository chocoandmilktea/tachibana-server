...
```
[premarket] 対象 158件 / 受領 158件 / 上限 158件 / 分割 80件 → （対象の銘柄コードをカンマ区切りで全件出力）
...
`[premarket] 起動時は窓外です。8:45を待機します` は収集窓の外で起動した場合の行で、
8:45〜9:06(JST/平日) の窓内に再起動した場合は代わりに `[premarket] 起動時に窓内のため即時開始します` が出ます。
