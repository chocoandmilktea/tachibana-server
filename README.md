...
取得して、Vercel API経由でフロント側に中継する常駐サーバーです。
Redisには直接繋がず、既存のVercelプロジェクトのAPI経由でやり取りするため、
このサーバー自体はRedisの認証情報を持ちません。

```
[立花証券 EVENT I/F(WebSocket)] ←常時接続— [このサーバー]
                                                 ↓ POST (5秒おきに間引き)
                          [Vercel: api/tachibana-quote.js] → [Upstash Redis]（sync.jsと共用）
                                                 ↑ GET
                    App.jsが7秒おきに api/tachibana-quote.js を取得
```
...
- まずは **デモ環境**（`TACHIBANA_ENV=demo`）で動作確認することを強く推奨します。
  本番環境は実際の口座に接続されます。
- `vercel-api/tachibana-watch.js` と `vercel-api/tachibana-quote.js` を、
  既存のVercelプロジェクトの `api/` フォルダに追加してデプロイしておいてください。
- Vercel側の環境変数に `TACHIBANA_RELAY_SECRET`（任意の英数字の合言葉）を追加し、
  Railway側の同名変数と同じ値にしてください（なりすまし防止用。空でも動作はします）。
...
3. このフォルダの中身（`index.js`, `package.json`, `railway.json`, `.gitignore`,
   ...
   `relay.js`, `watcher.js`）を、Filesアプリから複数選択してドラッグ＆ドロップ
...
npm install
cp .env.example .env
# .env を編集して、認証ID・秘密鍵・VercelのAPI URL・合言葉を入力してください
```
